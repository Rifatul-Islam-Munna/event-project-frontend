...
import { TableNodeData, Guest, TableType } from "../wedding-planner";
import { User, Trash2, Pencil, X, Plus } from "lucide-react";
...
const TableNodeInner = ({
  ...
  const [isSeatDragOver, setIsSeatDragOver] = useState<string | null>(null); // State to track which seat is being dragged over
  ...
  const handleDrop = useCallback(
    (event: React.DragEvent, seatId: string) => {
      ...
      setIsSeatDragOver(null); // Reset drag over state on drop
    },
    [onGuestDrop, id]
  );
  ...
  return (
    <div className="relative" style={{ width: width, height: height }}>
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <div
                    className={
                      `w-8 h-8 rounded-full border border-black flex items-center justify-center cursor-pointer transition-colors duration-200
                                  ${
                                    isOccupied
      ...
      {selected && (
        <div className="absolute -bottom-12 left-1/2 -translate-x-1/2 flex gap-1 transition-opacity duration-200">
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
